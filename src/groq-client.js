...
function buildPrompt({ query, urlData, priorMemory, personality }) {
  ...
  const priorContext = priorMemory
    ? `\n\nIMPORTANT: This summoner has visited you before (${priorMemory.visit_count} times). Their last query was: "${priorMemory.last_query}". You remember them. React to their return in some way - ask if they found what they were looking for, reference the previous conversation, or comment on the passage of time.`
    : '';
...
async function channelGhost({ query, urlData, priorMemory, apiKey }) {
  if (!apiKey || apiKey.trim() === '') {
    return { error: 'No Groq API key configured. Open Settings to add your key.' };
  }
